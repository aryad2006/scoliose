...
| **v3.2** | **Fév. 2026** | **à commit** | **Module admin complet (ADMIN_MODULE_SPECIFICATIONS.md), Module 1 rédigé (cours/)** |
...
| TECH-01 | Pas de stratégie SaMD pour SpineSim© | Section §21 ajoutée dans `SPINESIM_SPECIFICATIONS_TECHNIQUES.md` : Phase 1 « educational only », Phase 2 SaMD optionnelle avec critères de déclenchement | ✅ |
...
| XREF-03 | SpineSim v1.0 mois 40, intégration LMS mois 18 → MVS non défini | MVS défini dans `CALENDRIER_PRODUCTION.md` : features incluses (FEM, 11 pathologies, vis, WebGL) et exclues (VR, haptique, import DICOM) au mois 22 | ✅ |
...
| Dispositif médical | Classification SaMD (si SpineSim© clinique) | ✅ Phase 1 « educational only », Phase 2 SaMD optionnelle (§21 SpineSim) |
