...
## Plateforme LMS Formation Médicale avec SpineSim©
...
| Mesure | Détail |
|---|---|
| URL dédiée | `admin.spinesim.com` (sous-domaine séparé) |
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│  📊 DASHBOARD ADMIN — SpineSim© Platform                    🔔 5  👤 AM │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐ │
...
| Pipeline SpineSim | Sessions actives, queue FEM, utilisation GPU | 30 secondes |
...
│  [SpineSim] [Communications] [Activité] [Notes admin]          │
│                                                                 │
│  ── Onglet actif : PROFIL ──────────────────────────────────── │
│                                                                 │
│  Statut compte   : ✅ Actif                                    │
...
│  │  SpineSim    │  │  KneeSim?    │  │  SpineSim    │      │
│  └──────────────┘  └──────────────┘  └──────────────┘      │
│         │                 │                 │               │
│  ┌──────▼─────────────────▼─────────────────▼────────────┐  │
│  │              Catalogue de cours partagé                │  │
│  │  Modules communs : Anatomie, Biomécanique, Imagerie   │  │
│  │  SpineSim© partagé : même moteur, scénarios différents│  │
│  └───────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
```
...
| Correction manuelle | Cas cliniques ouverts, SpineSim → file d'attente correcteur |
...
| Mode dégradé | Option : lecture seule (accès au contenu mais pas aux quiz/SpineSim) |
...
| SpineSim (modèles, sessions) | Quotidien | 90 jours | S3 |
...
| GPU (SpineSim) | >85% | >95% | File d'attente des sessions FEM |
...
| Queue FEM (SpineSim) | >50 en attente | >100 en attente | Scale up GPU nodes |
...
| SpineSim | Sessions FEM, GPU usage, erreurs solver | 90 jours | ELK / Grafana |
...
| **Engagement SpineSim** | Sessions, durée, scénarios complétés, amélioration du score | CSV, PDF |
...
2. **Sélection des métriques** : inscriptions, complétion, score, temps, CA, sessions SpineSim
...
|---|---|---|
| Nom de la plateforme | Texte | « SpineSim© — Formation Médicale » |
| Logo | Image (SVG/PNG) | Logo SpineSim |
...
| Adresse d'envoi | noreply@spinesim.com |
| Reply-to | support@spinesim.com |
...
```
Base URL : https://api.spinesim.com/admin/v1
...
*Février 2026 — Formation Scoliose avec SpineSim©*
