...
<?php
/**
 * VERTEX© — Setup complet du cours "Obésité et Orthopédie" (VERTEX-OBO)
 *
 * Ce script :
 *  1. Nettoie le cours existant (suppression sections > 0 et activités)
 *  2. Crée 11 sections (0-10) avec les bons noms
 *  3. Importe les questions depuis le XML (76 questions)
 *  4. Crée les activités "page" (contenu MD→HTML) dans chaque section
 *  5. Crée les quiz par module (M01-M08) + quiz final
 *  6. Assigne les questions aux quiz par tag Module
 *
 * Usage : sudo -u www-data php setup_course_obo.php
 *         (ou depuis Docker : docker exec moodle-local-moodle-1 php /var/www/html/setup_course_obo.php)
 *
 * Prérequis : le cours VERTEX-OBO (id=6) doit exister.
...
$MODULE_FILES = [
    1 => 'CDC_OBO_01_FONDAMENTAUX_OBESITE.md',
    2 => 'CDC_OBO_02_TRAITEMENT_OBESITE.md',
    3 => 'CDC_OBO_03_BIOMECANIQUE_ARTICULAIRE.md',
    4 => 'CDC_OBO_04_ARTHROPLASTIE.md',
    5 => 'CDC_OBO_05_RACHIS.md',
    6 => 'CDC_OBO_06_TRAUMATOLOGIE.md',
    7 => 'CDC_OBO_07_PERIOPERATOIRE.md',
    8 => 'CDC_OBO_08_POPULATIONS_ETHIQUE.md',
];

// Cas cliniques
$CASE_FILES = [
    ['file' => 'CAS_CLINIQUES_OBO_P1.md', 'name' => 'Cas cliniques Bronze (7 cas)'],
    ['file' => 'CAS_CLINIQUES_OBO_P2.md', 'name' => 'Cas cliniques Argent (6 cas)'],
    ['file' => 'CAS_CLINIQUES_OBO_P3.md', 'name' => 'Cas cliniques Or et Diamant (7 cas)'],
];

// Noms des sections
$SECTIONS = [
    0  => 'Bienvenue',
    1  => 'M01 — Fondamentaux de l\'obésité',
    2  => 'M02 — Traitement de l\'obésité',
    3  => 'M03 — Biomécanique articulaire',
    4  => 'M04 — Arthroplastie',
    5  => 'M05 — Rachis',
    6  => 'M06 — Traumatologie',
    7  => 'M07 — Périopératoire',
    8  => 'M08 — Populations et éthique',
    9  => 'Cas cliniques progressifs',
    10 => 'Quiz final — Évaluation complète',
];

// Noms des quiz par module
$QUIZ_NAMES = [
    1 => 'Quiz M01 — Fondamentaux de l\'obésité',
    2 => 'Quiz M02 — Traitement de l\'obésité',
    3 => 'Quiz M03 — Biomécanique articulaire',
    4 => 'Quiz M04 — Arthroplastie',
    5 => 'Quiz M05 — Rachis',
    6 => 'Quiz M06 — Traumatologie',
    7 => 'Quiz M07 — Périopératoire',
    8 => 'Quiz M08 — Populations et éthique',
];

$QUIZ_FINAL_NAME = 'Quiz final — Obésité et Orthopédie';

$XML_FILE = $CONTENT_DIR . '/quiz_obesite-orthopedie.xml';
 ...
 * Retourne le numéro du module (1-8) ou 0 si non trouvé.
 */
...
function create_quiz_activity($course, $sectionnum, $name) {
    ...
    // Bits : 0x11011 = during + immediately after + later (attempt, correctness, marks, feedback, generalfeedback, rightanswer)
    $review_all = 0x11011;
    // Moodle 4.x review: each flag is a bitmask
    // DURING=0x10000, IMMEDIATELY=0x01000, OPEN=0x00100, CLOSED=0x00010, ALWAYS=0x00001
    // Pour tout montrer : immédiatement après + en revue + fermé
    $review_show = 0x11110;
...
echo "\n";
echo "  ═══════════════════════════════════════════════════════════════\n";
echo "  VERTEX© — Setup cours Obésité et Orthopédie (VERTEX-OBO)\n";
echo "  ═══════════════════════════════════════════════════════════════\n\n";
...
if (!$course) {
    out("[ERR] Cours VERTEX-OBO (id=$COURSE_ID) introuvable !");
...
if (!$files_ok) {
    ...
    out("        - quiz_obesite-orthopedie.xml");
    exit(1);
}
...
// ── 7. Créer les activités Page (modules 1-8) ──
echo "\n";
out("[*] Création des pages de contenu (M01-M08)...");
...
foreach ($MODULE_FILES as $num => $file) {
    ...
    $page_name = "Module M{$padnum} — " . $SECTIONS[$num];
    // Extraire juste le nom sans le préfixe M0X
...
// ── 8. Créer les quiz par module (M01-M08) ──
echo "\n";
...
// ── 9. Créer les pages de cas cliniques (section 9) ──
echo "\n";
out("[*] Création des cas cliniques (section 9)...");
...
foreach ($CASE_FILES as $cf) {
    ...
    $cm = create_page_activity($course, 9, $cf['name'], $html_content);
    if ($cm) {
...
// ── 10. Créer le quiz final (section 10) ──
echo "\n";
out("[*] Création du quiz final (section 10)...");

$quiz_final = create_quiz_activity($course, 10, $QUIZ_FINAL_NAME);
if (!$quiz_final) {
...
echo "  Sections    : " . count($SECTIONS) . " (0-10)\n";
echo "  Pages       : " . (count($MODULE_FILES) + count($CASE_FILES)) . " activités page\n";
echo "  Quiz module : 8 quiz (M01-M08)\n";
